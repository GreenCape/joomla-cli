 ...
 * If a key from the first array exists in the second array, its value will be replaced by the value from the second
 * array. If the key exists in the second array, and not the first, it will be created in the first array. If a key
 * only exists in the first array, it will be left as is. If several arrays are passed for replacement, they will be
 * processed in order, the later arrays overwriting the previous values. array_replace() is not recursive : it will
 * replace values in the first array by whatever type is in the second array.
 *
 ...
 * class="function">output_add_rewrite_var()</a></span> use the same Session module trans sid output buffer. Since PHP
 * 7.1.0, dedicated output buffer is used and {@see output_reset_rewrite_vars()} only removes rewrite vars defined by
