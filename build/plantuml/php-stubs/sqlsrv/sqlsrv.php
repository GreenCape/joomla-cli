 ...
 * using
 * these constants, see
 * {@link http://msdn.microsoft.com/en-us/library/ms173763(v=sql.110).aspx SET TRANSACTION ISOLATION LEVEL (Transact-SQL)}.<br />
 *
 * Additional Information at {@link http://msdn.microsoft.com/en-us/library/cc296152.aspx SQLSRV Driver API
 * Reference}<br />
 *
 ...
 * Repeatable Read.
 *
 * <br />Specifies that statements cannot read data that has been modified but not yet committed by other transactions
 ...
 * Other transactions can insert new rows that match the search conditions of statements issued by the current
 * transaction. If the current transaction then retries the statement it will retrieve the new rows, which results in
 * phantom reads. Because shared locks are held to the end of a transaction instead of being released at the end of
 * each statement, concurrency is lower than the default READ COMMITTED isolation level.<br />
 *
 ...
 * executed a second time, they will read the same set of rows. The range locks are held until the transaction
 * completes. This is the most restrictive of the isolation levels because it locks entire ranges of keys and holds the
 * locks until the transaction completes. Because concurrency is lower, use this option only when necessary. This
 * option has the same effect as setting HOLDLOCK on all tables in all SELECT statements in a transaction.<br />
 *
 * Used with the TransactionIsolation key when calling {@link sqlsrv_connect() sqlsrv_connect}. For information on
 * using
 * these constants, see
 ...
 *
 * During the roll-back phase of a database recovery, SNAPSHOT transactions will request a lock if an attempt is made
 * to
 ...
 * and {@link http://msdn.microsoft.com/en-us/library/cc296182.aspx How to: Connect Using SQL Server
 * Authentication.}<br />
 *
 * Additional Information at {@link http://msdn.microsoft.com/en-us/library/cc296152.aspx SQLSRV Driver API
 * Reference}<br />
 ...
 *                                   number (for example, "myServer, 1521") can be included as part of this string. For
 *                                   a complete description of the options available for this parameter, see the Server
 *                                   keyword in the ODBC Driver Connection String Keywords section of
 *                                   {@link http://go.microsoft.com/fwlink/?LinkId=105504 Using Connection String Keywords with SQL Native Client}.<br />
 *
 ...
 * Server support for AlwaysOn Availability Groups, see
 * {@link http://msdn.microsoft.com/en-us/library/hh487159.aspx PHP Driver for SQL Server Support for High Availability, Disaster Recovery}.
 * @param  array   $connection_info  [optional] An associative array that contains connection attributes (for example,
 *                                   array("Database" => "AdventureWorks")). See
 *                                   {@link http://msdn.microsoft.com/en-us/library/ff628167.aspx Connection Options}
 *                                   for a list of the supported keys for the array.
 *
 * @return resource|false A PHP connection resource. If a connection cannot be successfully created and opened, false
 *                        is returned.
 */
 ...
 *
 * <br />Null is a valid parameter for this function. This allows the function to be called multiple times in a script.
 * For example, if you close a connection in an error condition and close it again at the end of the script, the second
 * call to sqlsrv_close will return true because the first call to sqlsrv_close (in the error condition) sets the
 * connection resource to null.<br />
 *
 * Additional Information at {@link http://msdn.microsoft.com/en-us/library/cc296152.aspx SQLSRV Driver API
 * Reference}<br />
 *
 ...
 *
 * <br />Rolls back the current transaction on the specified connection and returns the connection to the auto-commit
 * mode. The current transaction includes all statements on the specified connection that were executed after the call
 * to sqlsrv_begin_transaction and before any calls to {@link sqlsrv_rollback() sqlsrv_rollback} or
 ...
 *                                    the
 *                                    following list: SQLSRV_ERR_ALL, SQLSRV_ERR_ERRORS, SQLSRV_ERR_WARNINGS. If no
 *                                    parameter value is supplied, both errors and warnings generated by the last
 ...
 * <li>For errors that originate from the Microsoft Drivers for PHP for SQL Server, the Microsoft Drivers for PHP for
 * SQL Server error code. For more information, see
 * {@link http://msdn.microsoft.com/en-us/library/cc626302.aspx Handling Errors and Warnings}.</li>
 * </ul>
 ...
 *                           information about client-side queries, see
 *                           {@link http://msdn.microsoft.com/en-us/library/hh487160.aspx Cursor Types (SQLSRV
 *                           Driver)}.
 *                           <ul>
 ...
 *                           For more information about logging activity, see
 *                           {@link http://msdn.microsoft.com/en-us/library/cc296188.aspx Logging Activity}.
 ...
 *                           For more information about configuring error and warning handling, see
 *                           {@link http://msdn.microsoft.com/en-us/library/cc626306.aspx How to: Configure Error and Warning Handling Using the SQLSRV Driver}.
 *                           <ul><li>true (1)</li>
 *                           <li>false (0)</li></ul>
 *
 ...
 *
 * <br />If false is returned by sqlsrv_get_config, you must call {@link sqlsrv_errors() sqlsrv_errors} to determine if
 * an error occurred or if false is the value of the setting specified by the $setting parameter.<br />
 *
 ...
 *
 * <br />Creates a statement resource associated with the specified connection. This function is useful for execution
 * of
 * multiple queries.<br />
 *
 ...
 *                             Each element of the array can be one of the following: a literal value, a reference to a
 *                             PHP variable, or an array with the following structure:
 ...
 *                             indicate the parameter direction: SQLSRV_PARAM_IN, SQLSRV_PARAM_OUT, SQLSRV_PARAM_INOUT.
 *                             The default value is SQLSRV_PARAM_IN. For more information about PHP constants, see
 *                             {@link http://msdn.microsoft.com/en-us/library/cc296183.aspx Constants (Microsoft Drivers for PHP for SQL Server)}.</li>
 *                             <li>$phpType[optional] - A SQLSRV_PHPTYPE_* constant that specifies PHP data type of the
 *                             returned value.</li>
 ...
 * amount of code. The sqlsrv_query function does both statement preparation and statement execution, and can be used
 * to
 ...
 *
 * <br />Makes the next row of a result set available for reading. Use {@link sqlsrv_get_field() sqlsrv_get_field} to
 * read fields of the row.<br />
 *
 ...
 *
 * @return array|null|false If the next row of the result set was successfully retrieved, true is returned. If there
 *                          are
 ...
 * specify the PHP return type, see
 * {@link http://msdn.microsoft.com/en-us/library/cc296208.aspx How to: Specify PHP Data Types}.) This combination of
 * functions also allows data to be retrieved as a stream. (For information about retrieving data as a stream, see
 * {@link http://msdn.microsoft.com/en-us/library/cc296155.aspx Retrieving Data as a Stream Using the SQLSRV
 * Driver}.)<br />
 *
 * Additional Information at {@link http://msdn.microsoft.com/en-us/library/cc296152.aspx SQLSRV Driver API
 * Reference}<br />
 ...
 *                                 {@link http://msdn.microsoft.com/en-us/library/cc296183.aspx Constants (Microsoft Drivers for PHP for SQL Server)}. If no return type is specified, a default PHP type will be returned. For information about default PHP types, see
 *                                 {@link http://msdn.microsoft.com/en-us/library/cc296193.aspx Default PHP Data
 *                                 Types}. For information about specifying PHP data types, see
 ...
 * For example, consider this Transact-SQL statement that inserts a value into a database table and retrieves the
 ...
 * If a result set contains multiple columns without names, the value of the last unnamed column will be assigned to
 * the
 * empty string ("") key.<br />
 *
 ...
 *                                     <li>SQLSRV_FETCH_BOTH - The next row of data is returned as both a numeric array
 *                                     and an associative array. This is the default value. </li></ul>
 ...
 *
 * @return array|null|false If a row of data is retrieved, an array is returned. If there are no more rows to retrieve,
 *                          null is returned. If an error occurs, false is returned.
 */
 ...
 *                                    for the
 *                                    $className parameter is not specified, an instance of the PHP {@link stdClass} is
 *                                    instantiated.
 ...
 *                                    the row to retrieve. The first record in the result set is 0.
 *
 * @return object|false|null A PHP object with properties that correspond to result set field names. Property values
 *                           are
 ...
 * data types, see {@link http://msdn.microsoft.com/en-us/library/cc296193.aspx Default PHP Data Types}.<br />
 */
 ...
 *
 * @return bool|null If the next result was successfully made active, the Boolean value true is returned. If an error
 *                   occurred in making the next result active, false is returned. If no more results are available,
 *                   null is returned.
 */
 ...
 *
 * <br />sqlsrv_num_rows requires a client-side, static, or keyset cursor, and will return false if you use a forward
 * cursor or a dynamic cursor. (A forward cursor is the default.) For more information about cursors, see
 ...
 *
 * @return int|bool False if there was an error calculating the number of rows. Otherwise, returns the number of rows
 *                  in the result set.
 */
 ...
 *
 * <br />Returns the number of rows modified by the last statement executed. This function does not return the number
 * of
 ...
 * <li>DriverVer - SQL Server Native Client DLL version: 10.50.xxx (Microsoft Drivers for PHP for SQL Server version
 * 2.0)</li>
 ...
 * example, if you free a statement in an error condition and free it again at the end of the script, the second call
 * to
 ...
 * encoding used to process the value if the value is a stream.<br />
 *
