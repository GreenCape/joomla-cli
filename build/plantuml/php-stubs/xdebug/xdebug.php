 ...
 * Function monitoring allows you to find out where in your code the functions that you provided as argument are called
 * from. This can be used to track where old, or, discouraged functions are used. The defined functions are case
 * sensitive, and a dynamic call to a static method will not be caught.
 *
...
/**
 * This function displays structured information about one or more variables that includes its type, value and refcount
 * information. Arrays are explored recursively with values. This function is implemented differently from PHP's
 * debug_zval_dump() function in order to work around the problems that that function has because the variable itself
 * is actually passed to the function. Xdebug's version is better as it uses the variable name to lookup the variable
 * in the internal symbol table and accesses all the properties directly without having to deal with actually passing a
 * variable to a function. The result is that the information that this function returns is much more accurate than
 * PHP's own function for showing zval information.
 *
 ...
 * error collection was started with xdebug_start_error_collection(). By default this function will not clear the error
 * collection buffer. If you pass true as argument to this function then the buffer will be cleared as well. This
 * function returns a string containing all collected errors formatted as an "Xdebug table".
 *
 * @param  bool  $clean
 *
 * @return string
 ...
 * current working directory. This current working directory might be different than you expect it to be, so please use
 * an absolute path in case you specify a file name. Use the PHP function getcwd() to figure out what the current
 ...
 * The information that is collected consists of an two dimensional array with as primary index the executed filename
 * and as secondary key the line number. The value in the elements represents the total number of execution units on
 * this line have been executed. Options to this function are: XDEBUG_CC_UNUSED Enables scanning of code to figure out
 * which line has executable code. XDEBUG_CC_DEAD_CODE Enables branch analyzes to figure out whether code can be
 * executed.
 *
 * @param  int  $options
 *
