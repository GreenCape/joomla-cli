...
class MongoClient
{
     ...
     *                                  ismaster database command on the seeds, so the driver may end up connecting to
     *                                  a
     ...
     *                                  database has flushed the changes to disk. This makes the write operations
     *                                  slower, but it guarantees that writes have succeeded and that the operations
     *                                  can be recovered in case of total system failure. If the MongoDB server has
     *                                  journaling enabled, this option is identical to "journal". If journaling is not
     *                                  enabled, this option ensures that write operations will be synced to database
     *                                  files on disk.
     *                                  "journal"
     ...
     *                                  slower, but it guarantees that writes have succeeded and that the operations
     *                                  can be recovered in case of total system failure. Note: If this option is used
     *                                  and journaling is disabled, MongoDB 2.6+ will raise an error and the write will
     *                                  fail; older server versions will simply ignore the option.
     *                                  "gssapiServiceName"
     *                                  Sets the » Kerberos service principal. Only applicable when
     *                                  authMechanism=GSSAPI. Defaults to "mongodb".
     *                                  "password"
     *                                  The password can be specified here, instead of including it in the host list.
     *                                  This is especially useful if a password has a "@" in it. This overrides a
     *                                  password set in the host list.
     *                                  "readPreference"
     *                                  Specifies the read preference type. Read preferences provide you with control
     *                                  from which secondaries data can be read from. Allowed values are:
     ...
     *                                  information.
     *                                  "replicaSet"
     *                                  The name of the replica set to connect to. If this is given, the primary will
     *                                  be automatically be determined. This means that the driver may end up
     *                                  connecting to a server that was not even listed. See the replica set example
     *                                  below for details.
     ...
     *                                  operations may block indefinitely. This option may also be set on a
     *                                  per-operation basis using MongoCursor::timeout() for queries or the
     *                                  "socketTimeoutMS" option for write methods. Note: This is a client-side
     *                                  timeout. If a write operation times out, there is no way to know if the server
     *                                  actually handled the write or not, as a MongoCursorTimeoutException will be
     *                                  thrown in lieu of returning a write result.
     *                                  "ssl"
     *                                  A boolean to specify whether you want to enable SSL for the connections to
     *                                  MongoDB. Extra options such as certificates can be set with SSL context
     *                                  options.
     *                                  "username"
     *                                  The username can be specified here, instead of including it in the host list.
     *                                  This is especially useful if a username has a ":" in it. This overrides a
     *                                  username set in the host list.
     *                                  "w"
     *                                  The w option specifies the Write Concern for the driver, which determines how
     *                                  long the driver blocks when writing. The default value is 1. This option is
     *                                  applicable when connecting to both single servers and replica sets. A positive
     *                                  value controls how many nodes must acknowledge the write instruction before the
     *                                  driver continues. A value of 1 would require the single server or primary (in a
     *                                  replica set) to acknowledge the write operation. A value of 3 would cause the
     *                                  driver to block until the write has been applied to the primary as well as two
     *                                  secondary servers (in a replica set). A string value is used to control which
     *                                  tag sets are taken into account for write concerns. "majority" is special and
     *                                  ensures that the write operation has been applied to the majority (more than
     *                                  50%) of the participating nodes.
     *                                  "wTimeoutMS" This option specifies the time limit, in milliseconds, for write
     *                                  concern acknowledgement. It is only applicable for write operations where "w"
     *                                  is greater than 1, as the timeout pertains to replication. If the write concern
     *                                  is not satisfied within the time limit, a MongoCursorException will be thrown.
     *                                  A value of 0 may be specified to block indefinitely. The default value is 10000
     *                                  (ten seconds).
     * @param  array   $driver_options  [optional] <p>
     *                                  An array of options for the MongoDB driver. Options include setting
     *                                  connection
     *                                  {@link http://php.net/manual/en/mongo.connecting.ssl.php#mongo.connecting.context.ssl context options for SSL} or {@link http://php.net/manual/en/context.mongodb.php logging callbacks}.
     *                                  </p><ul>
     *                                  <li>
     *                                  <p>
     *                                  <em>"context"</em>
     ...
     *                                      selected for writes would be closed. In a single-node configuration, that
     *                                      is then the whole connection, but if you are connected to a replica set,
     *                                      close() will only close the connection to the primary server. If connection
     *                                      is <b>TRUE</b> then all connections as known by the connection manager will
     *                                      be closed. This can include connections that are not referenced in the
     *                                      connection string used to create the object that you are calling close on.
     *                                      If connection is a string argument, then it will only close the connection
     ...
     *                                       <p>
     *                                       The ID of the cursor to kill. You can either supply an
     *                                       {@link http://www.php.net/manual/en/language.types.integer.php int}
     *                                       containing the 64 bit cursor ID, or an object of the
     *                                       {@link http://www.php.net/manual/en/class.mongoint64.php MongoInt64}
...
class MongoDB
{
     ...
     *                          </p><ul>
     *                          <li><p><em>"timeout"</em></p><p>Deprecated alias for
     *                          <em>"socketTimeoutMS"</em>.</p></li>
     *                          </ul>
     *
     ...
     * @param  array   $tags             [optional] <p>An array of zero or more tag sets, where each tag set is itself
     *                                   an array of criteria used to match tags on replica set members.</p>
     *
     ...
     *
     * @param  mixed  $w         <p>The write concern. This may be an integer denoting the number of servers required
     *                           to acknowledge the write, or a string mode (e.g. "majority").</p>
...
class MongoCollection
{
     ...
     *
     * @return MongoCommandCursor Returns a
     *                            {@link http://php.net/manual/en/class.mongocommandcursor.php MongoCommandCursor}
     *                            object
     */
     ...
     * @param  bool  $ok  [optional] <p>
     *                    If reads should be sent to secondary members of a replica set for all
     *                    possible queries using this
     *                    {@link http://www.php.net/manual/en/class.mongocollection.php MongoCollection} instance.
     *
     * @return bool Returns the former value of slaveOkay for this instance.
     * </p>
     */
     ...
     *
     * @param  array|object  $a        An array or object. If an object is used, it may not have protected or private
     *                                 properties. Note: If the parameter does not have an _id key or property, a new
     *                                 MongoId instance will be created and assigned to it. This special behavior does
     *                                 not mean that the parameter is passed by reference.
     ...
     * @param  array  $options   [optional] <p>An array of options for the remove operation. Currently available
     *                           options
     *                           include:
     *                           </p><ul>
     *                           <li><p><em>"w"</em></p><p>See
     *                           {@link http://www.php.net/manual/en/mongo.writeconcerns.php Write Concerns}. The
     *                           default value for <b>MongoClient</b> is <em>1</em>.</p></li>
     *                           <li>
     *                           <p>
     *                           <em>"justOne"</em>
     *                           </p>
     ...
     *                           <strong><code>FALSE</code></strong> or omitted, all documents matching the criteria
     *                           will be deleted.
     *                           </p>
     *                           </li>
     ...
     *                           will override setting <em>"w"</em> to <em>0</em>.</p><blockquote
     *                           class="note"><p><strong class="note">Note</strong>: <span class="simpara">If
     *                           journaling is enabled, users are strongly encouraged to use the <em>"j"</em> option
     *                           instead of <em>"fsync"</em>. Do not use <em>"fsync"</em> and <em>"j"</em>
     *                           simultaneously, as that will result in an error.</p></blockquote></li>
     *                           <li><p><em>"j"</em></p><p>Boolean, defaults to <b>FALSE</b>. Forces the write
     *                           operation to block until it is synced to the journal on disk. If
     *                           <strong><code>TRUE</code></strong>, an acknowledged write is implied and this option
     *                           will override setting <em>"w"</em> to <em>0</em>.</p><blockquote
     *                           class="note"><p><strong class="note">Note</strong>: <span class="simpara">If this
     *                           option is used and journaling is disabled, MongoDB 2.6+ will raise an error and the
     *                           write will fail; older server versions will simply ignore the
     *                           option.</p></blockquote></li>
     *                           <li><p><em>"socketTimeoutMS"</em></p><p>This option specifies the time limit, in
     *                           milliseconds, for socket communication. If the server does not respond within the
     *                           timeout period, a <b>MongoCursorTimeoutException</b> will be thrown and there will be
     *                           no way to determine if the server actually handled the write or not. A value of
     *                           <em>-1</em> may be specified to block indefinitely. The default value for
     *                           <b>MongoClient</b> is <em>30000</em> (30 seconds).</p></li>
     *                           <li><p><em>"w"</em></p><p>See
     *                           {@link http://www.php.net/manual/en/mongo.writeconcerns.php Write Concerns }. The
     *                           default value for <b>MongoClient</b> is <em>1</em>.</p></li>
     *                           <li><p><em>"wTimeoutMS"</em></p><p>This option specifies the time limit, in
     *                           milliseconds, for
     *                           {@link http://www.php.net/manual/en/mongo.writeconcerns.php write concern}
     *                           acknowledgement. It is only applicable when <em>"w"</em> is greater than <em>1</em>,
     *                           as the timeout pertains to replication. If the write concern is not satisfied within
     *                           the time limit, a <a href="class.mongocursorexception.php"
     *                           class="classname">MongoCursorException</a> will be thrown. A value of <em>0</em> may
     *                           be specified to block indefinitely. The default value for
     *                           {@link http://www.php.net/manual/en/class.mongoclient.php MongoClient} is
     *                           <em>10000</em> (ten seconds).</p></li>
     *                           </ul>
     *
     * <p>
     * The following options are deprecated and should no longer be used:
     * </p><ul>
     * <li><p><em>"safe"</em></p><p>Deprecated. Please use the
     * {@link http://www.php.net/manual/en/mongo.writeconcerns.php write concern} <em>"w"</em> option.</p></li>
     ...
     * @param  array  $options  An array of options to apply, such as remove the match document from the DB and return
     *                          it.
     *
     ...
     *
     * @param  array|object  $a        Array to save. If an object is used, it may not have protected or private
     *                                 properties. Note: If the parameter does not have an _id key or property, a new
...
class MongoCursor implements Iterator
{
     ...
     * @return array This function returns an array describing the read preference. The array contains the values
     *               <em>type</em> for the string read preference mode (corresponding to the
     *               {@link http://www.php.net/manual/en/class.mongoclient.php MongoClient} constants), and
     *               <em>tagsets</em> containing a list of all tag set criteria. If no tag sets were specified,
     *               <em>tagsets</em> will not be present in the array.
     */
    public function getReadPreference()
    {
    }
     ...
     *
     * @param  array  $fields  An array of fields by which to sort. Each element in the array has as key the field
     *                         name, and as value either 1 for ascending sort, or -1 for descending sort
     *
     * @throws MongoCursorException
     ...
     *
     * @param  int  $batchSize  The number of results to return per batch. Each batch requires a round-trip to the
     *                          server.
     *                          <p>If batchSize is 2 or more, it represents the size of each batch of objects
     *                          retrieved.
...
class MongoGridFSFile
{
     ...
     *
     * @param  string  $filename  The location to which to write the file (path+filename+extension). If none is given,
...
class MongoId
{
     ...
     *
     * @param  string  $id  [optional] A string to use as the id. Must be 24 hexidecimal characters. If an invalid
     *                      string is passed to this constructor, the constructor will ignore it and create a new id
     *                      value.
     *
...
class MongoWriteBatch
{
     ...
     *                                          will be copied and used as the default write concern if none is given
     *                                          as <code >$write_options</code> or during
     ...
     *                                          <p> An array of Write
     *                                          Options.</p><table><thead><tr><th>key</th><th>value meaning</th></tr>
     *                                          </thead>
     *                                          <tbody><tr><td>w
     *                                          (int|string)</td><td>{@link http://php.net/manual/en/mongo.writeconcerns.php Write concern} value</td></tr>
     *                                          <tr><td>wtimeout
     *                                          (int)</td><td>{@link http://php.net/manual/en/mongo.writeconcerns.php Maximum time to wait for replication}</td></tr>
     *                                          <tr><td>ordered</td><td>Determins if MongoDB must apply this batch in
     *                                          order (sequentally, one item at a time) or can rearrange it. Defaults
     *                                          to <strong><code>TRUE</code></strong></td></tr>
     *                                          <tr><td>j (bool)</td><td>Wait for journaling on the primary. This value
     *                                          is discouraged, use WriteConcern instead</td></tr>
     *                                          <tr><td>fsync (bool)</td><td>Wait for fsync on the primary. This value
     *                                          is discouraged, use WriteConcern instead</td></tr>
     *                                          </tbody></table>
     */
...
class MongoUpdateBatch extends MongoWriteBatch
{
     ...
     *                                          if none is given as $write_options or during
     *                                          {@see MongoWriteBatch::execute()}.</p>
...
class MongoLog
{
     ...
     * (PECL mongo &gt;= 1.3.0)<br/>
     * <p>
     * This function will set a callback function to be called for
     * {@link http://www.php.net/manual/en/class.mongolog.php MongoLog} events instead of triggering warnings.
     * </p>
     *
     ...
     *
     * <p>One of the
     * {@link http://www.php.net/manual/en/class.mongolog.php#mongolog.constants.module MongoLog module constants}.</p>
     * </li>
     * <li>
     * <b><i>level</i></b>
     *
     * <p>One of the
     * {@link http://www.php.net/manual/en/class.mongolog.php#mongolog.constants.level MongoLog level constants}.</p>
     * </li
     * <li>
     * <b><i>message</i></b>
     *
