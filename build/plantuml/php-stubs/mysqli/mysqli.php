...
class mysqli
{
     ...
     *                            connection. mysqli_change_user() is automatically called on connections opened from
     *                            the connection pool. Defaults to ini_get("mysqli.default_host")
     ...
     *                            user against those user records which have no password only. This allows one username
     *                            to be used with different permissions (depending on if a password as provided or
     *                            not). Defaults to ini_get("mysqli.default_pw")
 ...
 * no rows matched the WHERE clause in the query or that no query has yet been executed. -1 indicates that the query
 * returned an error.
 */
 ...
 *                                 mysqli_store_result() or mysqli_use_result().
 *
 * @return object|bool Returns an object which contains field definition information or FALSE if no field information
 *                     is available.
 */
 ...
 * where each key in the array represents the name of one of the result set's columns or NULL if there are no more rows
 * in resultset. If two or more columns of the result have the same field names, the last column will take precedence.
 * To access the other column(s) of the same name, you either need to access the result with numeric indices by using
 ...
 *                                     objects.
 *
 * @return object|null Returns an object with string properties that corresponds to the fetched row or NULL if there
 *                     are no more rows in resultset. If two or more columns of the result have the same field names,
 *                     the last column will take precedence. To access the other column(s) of the same name, you either
 *                     need to access the result with numeric indices by using mysqli_fetch_row() or add alias names.
 */
 ...
 * The form of this version number is main_version * 10000 + minor_version * 100 + sub_version (i.e. version 4.1.0 is
 * 40100).
 */
 ...
 *                    there was no previous query on the connection or if the query did not update an AUTO_INCREMENT
 *                    value. If the number is greater than maximal int value, mysqli_insert_id() will return a string.
 */
 ...
 *
 * @return bool Returns FALSE if the first statement failed. To retrieve subsequent errors from other statements you
 *              have to call mysqli_next_result() first.
 */
 ...
 * @param  mysqli_stmt  $stmt
 *
 * @return int|string If the number of affected rows is greater than maximal PHP int value, the number of affected rows
 ...
 * @param  mysqli  $link  A link identifier returned by mysqli_connect() or mysqli_init()
 *
 * @return string Returns a string containing the SQLSTATE error code for the last error. The error code consists of
 *                five characters. '00000' means no error.
 */
