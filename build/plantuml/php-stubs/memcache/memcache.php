...
class MemcachePool
{
     ...
     *                           {@link http://php.net/manual/ru/memcache.ini.php#ini.memcache.default-port memcache.default_port} if not specified. For this reason it is wise to specify the port explicitly in this method call.
     *                           </p>
     * @param  int     $timeout  [optional] <p>Value in seconds which will be used for connecting to the daemon. Think
     *                           twice before changing the default value of 1 second - you can lose all the advantages
     *                           of caching if your connection is too slow.</p>
     *
     ...
     *                                      <i>port</i> must also be set to 0.
     *                                      </p>
     * @param  int       $port              [optional] <p>
     ...
     * @param  int       $timeout           [optional] <p>
     *                                      Value in seconds which will be used for connecting to the daemon. Think
     *                                      twice before changing the default value of 1 second - you can lose all the
     ...
     *                                      {@link http://www.php.net/manual/en/function.dl.php dl()}.
     *                                      </p>
     * @param  bool      $status            [optional] <p>
     *                                      Controls if the server should be flagged as online. Setting this parameter
    ...
    /**
     * (PECL memcache &gt;= 2.0.0)<br/>
     * Add an item to the server. If the key already exists, the value will not be added and <b>FALSE</b> will be
     * returned.
     *
     ...
     *                          You can also use Unix timestamp or a number of seconds starting from current time, but
     *                          in the latter case the number of seconds may not exceed 2592000 (30 days).</p>
     *
     * @return boolean Returns <b>TRUE</b> on success or <b>FALSE</b> on failure. Returns <b>FALSE</b> if such key
     *                 already exist. For the rest Memcache::add() behaves similarly to Memcache::set().
     */
     ...
     * @param  int     $expire  [optional] Expiration time of the item. If it's equal to zero, the item will never
     *                          expire. You can also use Unix timestamp or a number of seconds starting from current
     *                          time, but in the latter case the number of seconds may not exceed 2592000 (30 days).
     *
     ...
     *                          serialized.</p>
     * @param  int     $flag    [optional] <p>Use <b>MEMCACHE_COMPRESSED</b> to store the item compressed (uses
     *                          zlib).</p>
     ...
     *                               These flags are the same as the ones given to for example
     *                               {@link http://php.net/manual/en/memcache.set.php Memcache::set()}. The lowest byte
     *                               of the int is reserved for pecl/memcache internal usage (e.g. to indicate
     *                               compression and serialization status).
     *                               </p>
     *
     * @return string|array <p>
     * Returns the string associated with the <b>key</b> or
     * an array of found key-value pairs when <b>key</b> is an
     * {@link http://php.net/manual/en/language.types.array.php array}. Returns <b>FALSE</b> on failure, <b>key</b> is
     * not found or
     ...
     *                          According to the memcached protocol spec these additional arguments "are subject to
     *                          change for the convenience of memcache developers".</p>
     ...
     *                          cachedump, slabs, items, sizes}. According to the memcached protocol spec these
     *                          additional arguments "are subject to change for the convenience of memcache
     *                          developers".</p>
     * @param  int     $slabid  [optional] <p>
     *                          Used in conjunction with <b>type</b> set to
     *                          cachedump to identify the slab to dump from. The cachedump
     ...
     *
     * @param  int    $thresold    <p>Controls the minimum value length before attempting to compress
     *                             automatically.</p>
     * @param  float  $min_saving  [optional] <p>Specifies the minimum amount of savings to actually store the value
     *                             compressed. The supplied value must be between 0 and 1. Default value is 0.2 giving
     *                             a minimum 20% compression savings.</p>
     *
     * @return boolean Returns <b>TRUE</b> on success or <b>FALSE</b> on failure.
     */
