 ...
 *   * `"json"` - encodes objects and arrays as JSON object (using `json_encode()`), primitives written in stringified
 *   form, which is allowed for most of the JSON parsers as valid values. For empty arrays JSON array preferred, if it
 *   is necessary, use `new stdClass()` to persist empty JSON object. Note, that only JSON format considered supported
 *   by all Couchbase SDKs, everything else is private implementation (i.e. `"php"` format won't be readable by .NET
 *   SDK).
 *   * `"php"` - uses PHP serialize() method to encode the document.
 *   * `"igbinary"` - uses pecl/igbinary to encode the document in even more efficient than `"php"` format. Might not
 *   be
 ...
 *
 *   controls the form of the documents, returned by the server if they were in JSON format. When true, it will
 *   generate
...
namespace Couchbase {
    ...
    final class ClusterManager
    {
         ...
         *
         * Returns an associative array of status information as seen on the cluster.  The exact structure of the
         * returned data can be seen in the Couchbase Manual by looking at the cluster /info endpoint.
         *
         * @return array
         *
         ...
         * @param  UserSettings  $settings  settings (credentials and roles)
         * @param  int           $domain    RBAC domain
         *
    ...
    final class Bucket
    {
         ...
         *
         * @return Document|array document or list of the documents
         *
         ...
         * @param  array         $options  options
         *                                 * "expiry" document expiration time in seconds. If larger than 30 days
         *                                 (60*60*24*30), it will be interpreted by the server as absolute UNIX time
         *                                 (seconds from epoch
         *                                 1970-01-01T00:00:00).
         *                                 * "persist_to" how many nodes the key should be persisted to (including
         *                                 master). If set to 0 then persistence will not be checked. If set to a
         *                                 negative number, will be set to the maximum number of nodes to which
         *                                 persistence is possible (which will always contain at least the master
         *                                 node).
         *                                 * "replicate_to" how many nodes the key should be persisted to (excluding
         *                                 master). If set to 0 then replication will not be checked. If set to a
         *                                 negative number, will be set to the maximum number of nodes to which
         *                                 replication is possible (which may be 0 if the bucket is not configured for
         *                                 replicas).
         ...
         *
         * @param  string|array  $ids      one or more IDs
         * @param  int           $delta    the number whih determines the sign (positive/negative) and the value of the
         *                                 increment
         * @param  array         $options  options
         ...
         * @param  bool                                                             $jsonAsArray  if true, the values
         *                                                                                        in the result rows
         *                                                                                        (or hits) will be
         *                                                                                        represented as PHP
         *                                                                                        arrays, otherwise
    ...
    final class SearchQuery implements JsonSerializable
    {
         ...
         */
        final public static function geoBoundingBox(
            $topLeftLongitude,
            $topLeftLatitude,
    ...
    final class DateRangeSearchQuery implements JsonSerializable, SearchQueryPart
    {
         ...
         *                             time formatter (see dateTimeParser). Integers interpreted as unix timestamps and
         *                             represented as RFC3339 strings.
    ...
    /**
     * A FTS query that matches on Couchbase document IDs. Useful to restrict the search space to a list of keys (by
     * using this in a compound query).
     */
    ...
    /**
     * A FTS query that matches several terms (a "phrase") as is. The order of the terms mater and no further
     * processing is applied to them, so they must appear in the index exactly as provided.  Usually for debugging
     * purposes, prefer MatchPhraseQuery.
     */
