...
class ZendAPI_Queue
{
     ...
     *                                  application id (unless explicitly specified otherwise). I.e. When adding new
     *                                  job, unless this job already set an application id, the job will be assigned
     *                                  the queue application id
     *
     * @return bool Success
     */
     ...
     *
     * @param  array  $filter_options  Array of optional filter options to filter the jobs we want to get from the
     *                                 queue. If not set, all jobs will be counted.<br> Options can be: priority,
     *                                 application_id, host, name, status, recurring.
     *
 ...
 *
 * @param  event_handler_func string The callback function that will be call when the event is triggered, object
 *                                   methods may also be invoked statically using t his function by passing
 *                                   array($objectname, $methodname) to the function parameter
 * @param  handler_register_name string[optional] The name this function is registered under - if none is supplied, the
 *                                                function will be registerd under it's own name
 * @param  event_type_mask int The mask of event types that the handler should be called on by default it's set to
 *                             MONITOR_EVENT_ALL.
 *
 ...
 * @param  custom_headers string[optional] user defined headers that will be send instead of regular ZDS headers. few
